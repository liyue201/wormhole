...
export function getIsTransferCompletedTerra(tokenBridgeAddress, signedVAA, client, gasPriceUrl) {
    return __awaiter(this, void 0, void 0, function () {
        var msg, gasPrices, account, e_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, redeemOnTerra(tokenBridgeAddress, TERRA_REDEEMED_CHECK_WALLET_ADDRESS, signedVAA)];
                case 1:
                    msg = _a.sent();
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    BIG_MAX_BITS = BigInt(MAX_BITS);
                    BIG_EIGHT = BigInt(8);
                    start = (seq / BIG_MAX_BITS) * BIG_MAX_BITS;
                    beg = safeBigIntToNumber(seq - start);
                    s = Math.floor(beg / BITS_PER_KEY);
                    b = Math.floor((beg - s * BITS_PER_KEY) / 8);
                    key = Buffer.from(bigIntToBytes(s, 1)).toString("base64");
                    appState.forEach(function (kv) {
                        if (kv["key"] === key) {
                            var v = Buffer.from(kv["value"]["bytes"], "base64");
                            var bt = 1 << safeBigIntToNumber(seq % BIG_EIGHT);
                            retval = (v[b] & bt) != 0;
                            return;
                        }
                    });
...
export function getIsTransferCompletedAlgorand(client, appId, signedVAA) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    parsedVAA = _parseVAAAlgorand(signedVAA);
                    seq = parsedVAA.get("sequence");
                    chainRaw = parsedVAA.get("chainRaw");
                    em = parsedVAA.get("emitter");
                    return [4 /*yield*/, calcLogicSigAccount(client, appId, seq / BigInt(MAX_BITS), chainRaw + em)];
                case 1:
